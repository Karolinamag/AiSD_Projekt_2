...
void SortowaniePrzezScalanie(int Tablica[],int lewy, int prawy //Przyjmuje Tablice, lwey i prawy koniec
{
...
void testy (int Tablica[]) //Przyjmuje pusta zmienna dla tablic
{

    fstream zapisywanie; //Tworzenie zmiennej, ktora bedzie przekazywaæ dane do pliku, typu do tego przeznaczonego
    zapisywanie.open("Czasy.txt", ios::out); //Otwieranie nowego pliku tekstowego do ktorego zapisywane beda wyniki
    ...
    for (int ile=10; ile<100000; ile+=10000){ //For ktory wykonuje sie 10 razy, wartosc iteratora jest na raz tez dlugoscia tablicy ktora kazemy wygenerowaæ programowi. Jest ona tak duza bo co dopiero taka dlugosæ daje jakikolwiek czas dzialania
        ...
        cout<<"Proba dla "<<ile<<" elementow dla sortowania bombelkowego. Czas wykonania: "<<BombelkoweCzas.count()<<"ms"<<endl; //Wyswietlanie wynikow
...
int main()
{
    ...
    testy(Tablica); //Wywolywanie testow
