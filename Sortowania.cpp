...
fstream zapisywanie; //Tworzenie zmiennej, ktora bedzie przekazywaæ dane do pliku, typu do tego przeznaczonego
    zapisywanie.open("WygenerowanaTablica.txt", ios::out); //Otwieranie nowego pliku tekstowego do ktorego zapisywany bedzie wynik
    zapisywanie<<"WygenerowanaTablica = [ "; //Zamiast "cout" mamy zmienna "zapisywanie" wiec wszytko idzie do pliku txt
    for (int j=0; j<N; j++){
...
void TestyPrzemienne (int Tablica[]) //Przyjmuje pusta zmienna dla tablic
{
    ...
    fstream zapisywanie; //Tworzenie zmiennej, ktora bedzie przekazywaæ dane do pliku, typu do tego przeznaczonego
    zapisywanie.open("CzasyPrzemienne.txt", ios::out); //Otwieranie nowego pliku tekstowego do ktorego zapisywane beda wyniki
...
int main()
{
    ...
    zapisywanie<<"Tablica po sortowaniu = [ "; //Zamiast "cout" mamy zmienna "zapisywanie" wiec wszytko idzie do pliku txt
    for (int j=0; j<sizeof(Tablica); j++){
    ...
    cout<<"2. Pomiary czasow dzialania:"<<endl<<endl;
    ...
    cout<<endl<<"2.3 Pomiar dla tablicy posortowanej ale zaczynajacej sie od najwiekzej liczby:"<<endl<<endl;

    TestyOdwrotniePosortowane(Tablica);

     cout<<endl<<"2.4 Pomiar dla tablicy z wpisanymi najpierw liczbami nieparzystymi a potem parzystymi:"<<endl<<endl;
